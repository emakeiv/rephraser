...
def encode_token():
    ...
    private_key_text = (Path(__file__).parent / "keys/private_key.pem").read_text().encode("utf-8")
    private_key = serialization.load_pem_private_key(private_key_text,password=None)
    return jwt.encode(payload=payload, key=private_key, algorithm="RS256")


def decode_token(access_token):
    public_key_text = (Path(__file__).parent / "keys/public_key.pem").read_text().encode("utf-8")
